from __future__ import annotations
from dataclasses import dataclass
from typing import Dict, Tuple, List

MAX_CHAMP_LEVEL = 18  # LoL levels 1–18


@dataclass
class MonsterScaling:
    """
    Base -> max scaling for a monster, as a function of "monster level".
    We'll drive monster level by average champion level.
    """
    hp: Tuple[float, float]
    armor: Tuple[float, float]
    mr: Tuple[float, float]
    attack_damage: Tuple[float, float]
    attack_speed: float
    gold: Tuple[float, float]
    xp: Tuple[float, float]
    notes: str = ""


MONSTER_SCALING: Dict[str, MonsterScaling] = {
    "Blue Sentinel": MonsterScaling(
        hp=(2300.0, 6210.0),
        armor=(42.0, 42.0),
        mr=(42.0, 42.0),
        attack_damage=(66.0, 198.0),
        attack_speed=0.493,
        gold=(90.0, 90.0),
        xp=(95.0, 142.5),
        notes="large_monster; approximated from live stats.",
    ),
    "Murk Wolves (Greater)": MonsterScaling(
        hp=(1600.0, 3760.0),
        armor=(42.0, 42.0),
        mr=(42.0, 42.0),
        attack_damage=(30.0, 90.0),
        attack_speed=0.625,
        gold=(55.0, 55.0),
        xp=(50.0, 75.0),
        notes="large_monster.",
    ),
    "Gromp": MonsterScaling(
        hp=(2050.0, 4817.5),
        armor=(42.0, 42.0),
        mr=(42.0, 42.0),
        attack_damage=(70.0, 210.0),
        attack_speed=0.425,
        gold=(80.0, 80.0),
        xp=(120.0, 180.0),
        notes="large_monster.",
    ),
    "Crimson Raptor": MonsterScaling(
        hp=(1200.0, 2820.0),
        armor=(42.0, 42.0),
        mr=(42.0, 42.0),
        attack_damage=(17.0, 51.0),
        attack_speed=0.667,
        gold=(35.0, 35.0),
        xp=(20.0, 30.0),
        notes="large_monster.",
    ),
    "Ancient Krug": MonsterScaling(
        hp=(1350.0, 3172.5),
        armor=(42.0, 42.0),
        mr=(42.0, 42.0),
        attack_damage=(57.0, 171.0),
        attack_speed=0.613,
        gold=(15.0, 15.0),  # camp total gold/xp is higher; this is per unit
        xp=(15.0, 22.5),
        notes="large_monster; splits into smaller Krugs.",
    ),
    "Red Brambleback": MonsterScaling(
        hp=(2300.0, 6210.0),
        armor=(42.0, 42.0),
        mr=(42.0, 42.0),
        attack_damage=(66.0, 198.0),
        attack_speed=0.493,
        gold=(90.0, 90.0),
        xp=(95.0, 142.5),
        notes="large_monster.",
    ),
    "Rift Scuttler": MonsterScaling(
        hp=(1550.0, 3425.5),
        armor=(42.0, 42.0),
        mr=(42.0, 42.0),
        attack_damage=(35.0, 35.0),  # mostly flat; adjust later if needed
        attack_speed=0.638,
        gold=(55.0, 121.0),
        xp=(100.0, 150.0),
        notes="large_monster; ignores special shield mechanics in sim.",
    ),
    # TODO: Add dragons, herald, baron, Atakan, etc.
}


def monster_level_from_champs(champ_levels: List[int]) -> int:
    """
    Compute jungle monster 'level' as the average player level,
    rounded to the nearest integer, clamped to [1, MAX_CHAMP_LEVEL].
    """
    if not champ_levels:
        return 1
    avg = sum(champ_levels) / len(champ_levels)
    lvl = int(round(avg))
    return max(1, min(MAX_CHAMP_LEVEL, lvl))


def lerp_stat(base: float, max_val: float, level: int, max_level: int = MAX_CHAMP_LEVEL) -> float:
    """
    Linearly interpolate stat between base and max_val as level goes from 1 to max_level.
    """
    if max_level <= 1 or base == max_val:
        return base
    t = (level - 1) / (max_level - 1)  # 0 at level 1, 1 at level max_level
    return base + t * (max_val - base)


def get_monster_stats(camp_name: str, champ_levels: List[int]) -> dict:
    """
    Compute the stats of a jungle camp 'camp_name' given the
    current champion levels in the game.

    Returns a simple dict you can feed into CampStats / your sim.
    """
    if camp_name not in MONSTER_SCALING:
        raise KeyError(f"Unknown camp: {camp_name}")

    scaling = MONSTER_SCALING[camp_name]
    lvl = monster_level_from_champs(champ_levels)

    hp = lerp_stat(scaling.hp[0], scaling.hp[1], lvl)
    armor = lerp_stat(scaling.armor[0], scaling.armor[1], lvl)
    mr = lerp_stat(scaling.mr[0], scaling.mr[1], lvl)
    ad = lerp_stat(scaling.attack_damage[0], scaling.attack_damage[1], lvl)
    atk_spd = scaling.attack_speed
    gold = lerp_stat(scaling.gold[0], scaling.gold[1], lvl)
    xp = lerp_stat(scaling.xp[0], scaling.xp[1], lvl)

    return {
        "level": lvl,
        "hp": hp,
        "armor": armor,
        "mr": mr,
        "attack_damage": ad,
        "attack_speed": atk_spd,
        "gold": gold,
        "xp": xp,
        "notes": scaling.notes,
    }
