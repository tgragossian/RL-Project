...
class PlayerState:
    """
    Very simple player state holder for gold/items/resources.
    """
    def __init__(self, gold: float, items: list, health: float, mana: float):
        self.gold = gold
        self.items = items
        self.health = health
        self.mana = mana
...
# ---------------- Lane abstraction + heuristic policy ---------------- #
...
class LaneState:
    ...
    All numbers are normalized to [0, 1].
    """
    lane_type: LaneType
    win_prob: float
    ...
    def clamped(self) -> "LaneState":
        """
        Return a copy with continuous values clamped into [0, 1].
        """
        return LaneState(
...
class JungleHeuristicPolicy:
    ...
    Not RL – just a baseline "expert" heuristic.
    """
    ...
    def __init__(
        ...
        min_dive_win_prob: float = 0.52,   # currently unused, but kept for tuning
    ...
    def gank_score(self, lane: LaneState) -> float:
        ...
        # 1. How favorable is the fight?
        score += 3.0 * (lane.win_prob - 0.5)

        # 2. Enemy low HP is good for ganks.
        ...
        # 5. Side lanes a bit more attractive than mid.
        if lane.lane_type in (LaneType.TOP, LaneType.BOT):
            score += 0.3

        # 6. Bonus for “free kill” cleanup.
    ...
    def choose_action(
        ...
        # Safety: too low HP or can't gank right now.
